...
const Doctors = () => {
  ...
  return (
    ...
    {specialityData.map((item)=>(
      <button className='w-45 bg-blue-50 flex gap-1 py-2 rounded px-1 transition-all duration-500 ease  hover:bg-transparent hover:border-[#5f6fff] border border-[#00000] cursor-pointer' onClick={()=>{
      ...
      <div className='w-full flex justify-center flex-wrap gap-4  gap-y-6 px-3 sm:px-0 '>
             {doctor.map((item,idxNum)=>(
                 <div key={idxNum} className='border w-60 border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500 ease'
                  onClick={()=>{
